...
ha4 px0 py0 vx0 vy0 = switch (traj px0 py0 vx0 vy0) ssfunc
  where  traj px' py' vx' vy' = proc input -> do
               ...
               ev3 <- edge -< py <= -10
               ev4 <- edge -< py >= 15
               returnA -< ((px,py,vx,vy),if(ev1 /= NoEvent || ev2 /= NoEvent)  
                                           then if(ev3 /= NoEvent || ev4 /= NoEvent)
                                                  then Event() `tag` (px,py,-vx,-vy)
                                                  else Event() `tag` (px,py,-vx,vy)
                                           else if(ev3 /= NoEvent || ev4 /= NoEvent)
                                                  then Event() `tag` (px,py,vx,-vy)
