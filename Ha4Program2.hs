...
ha4 :: Double -> Double -> Double -> Double -> SF () (Double,Double,Double,Double)
ha4 px0 py0 vx0 vy0 = switch (traj px0 py0 vx0 vy0) ssfunc
  where  traj px' py' vx' vy' = proc input -> do
               ...
               ev3 <- edge -< py <= -10
               ev4 <- edge -< py >= 15
               returnA -< ((px,py,vx,vy),if(ev1 /= NoEvent) then Event() `tag` (px,py,-vx,vy) 
                                                            else if(ev2 /= NoEvent) then Event() `tag` (px,py,-vx,vy)
                                                                                    else if(ev3 /= NoEvent) then Event() `tag` (px,py,vx,-vy)
                                                                                                            else if(ev4 /= NoEvent) then Event() `tag` (px,py,vx,-vy)
                                                                                                                                    else NoEvent `tag` (px,py,vx,vy))                                                                                             
         ssfunc (px',py',vx',vy') = ha4 px' py' vx' vy'
...
simulate = discardInputs >>> (ha4 (-10) (-10) 40 10) >>> draw
